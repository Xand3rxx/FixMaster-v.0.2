...
<style>
table thead th:nth-child(22), table tbody td:nth-child(22) {
    position: sticky;
    right: 0;
}
      ...
      <tr>
        ...
        <td class="tx-medium">{{$payment['service_request']['unique_id']}}</td>
          ...
          @foreach($payment['users']['roles'] as $role)
            ...
            {{-- for CSEs --}}
                <td class="tx-medium">{{$payment['users']['account']['first_name'].' '.$payment['users']['account']['middle_name'].' '.$payment['users']['account']['last_name'] ?? 'Nil'}}</td>
                <td class="tx-medium">{{!empty($payment['users']['account']['account_number']) ? $payment['users']['account']['account_number'] : 'Nil'}}</td>
                <td class="tx-medium">{{!empty($payment['users']['account']['bank']['name']) ? $payment['users']['account']['bank']['name'] : 'Nil'}}</td>
                <td class="tx-medium">&#8358;{{!empty($payment['flat_rate']) ? number_format($payment['flat_rate'],2) : 'Nil'}}</td>
            @else
            <td class="tx-medium">Nil</td>
            <td class="tx-medium">Nil</td>
            <td class="tx-medium">Nil</td>
            <td class="tx-medium">Nil</td>
            @endif
          ...
          {{-- for Technicians --}}
            ...
            @endif
              ...
              @endif
          @endforeach
          <td class="text-medium tx-center">{{ Carbon\Carbon::parse($payment->updated_at, 'UTC')->isoFormat('MMMM Do YYYY, h:mm:ssa') }}</td>
