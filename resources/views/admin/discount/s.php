...
$(document).ready(function() {
    ...
    $('.selectpicker.select-all-service').on('changed.bs.select', function (e, clickedIndex, isSelected, previousValue) {
            if(e?.target?.options[clickedIndex]){
			if(e.target.options[clickedIndex].selected){
                catid.push(e.target.options[clickedIndex].value)				
			}else{
                catid = jQuery.grep(catid, function(value) {
                return value != e.target.options[clickedIndex].value;
                });   
            }
            }
