...
@if(empty(Auth::user()['account']['avatar']))
    <img src="{{ asset('assets/user-avatars/'.$genderAvatar) }}" class="rounded-circle" alt="Default avatar">
@elseif(!file_exists(public_path('assets/user-avatars/'.Auth::user()['account']['avatar'])))
    <img src="{{ asset('assets/user-avatars/'.$genderAvatar) }}" class="rounded-circle" alt="Profile avatar">
@else
